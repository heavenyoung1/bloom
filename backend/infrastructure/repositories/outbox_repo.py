...
from backend.infrastructure.models.outbox import OutboxORM, OutboxStatus, OutboxEventType
...
class OutboxRepository(IOutboxRepository):
    ...
    async def mark_as_completed(
        ...
        try:
            ...
            raise DatabaseErrorException(f'Ошибка при пометке события как completed: {e}')
    ...
    async def increment_retry_count(
        ...
        try:
            ...
            logger.debug(f'[OUTBOX] Счетчик попыток для события {event_id}: {new_count}')
            return new_count
