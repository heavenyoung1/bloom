...
class OutboxWorker:
        ...
        try:
            ...
            if event.event_type == OutboxEventType.ATTORNEY_REGISTERED.value:
                ...
                await uow.outbox_repo.mark_as_failed(
                    event.id, f'Неизвестный тип события: {event.event_type}'
                )
                await uow.commit()
                return
    ...
    async def _handle_attorney_registered(self, event) -> None:
        ...
        try:
            ...
            logger.info(f'[OUTBOX WORKER] Отправка кода верификации для {email}')
            ...
            logger.info(f'[OUTBOX WORKER] Код верификации отправлен для {email}')
