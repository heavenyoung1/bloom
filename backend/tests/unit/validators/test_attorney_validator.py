...
class TestAttorneyValidator:
    ...
    @pytest.mark.asyncio
    async def test_validate_on_create_license_id_taken(self, attorney_validator, attorney_repo_mock, valid_attorney_dto):
        ...
        # Проверяем, что выбрасывается исключение для email
        with pytest.raises(ValidationException, match=f'Номер Удостоверения адвоката {valid_attorney_dto.license_id} уже занят'):
            await attorney_validator.validate_on_create(valid_attorney_dto)

        attorney_repo_mock.get_by_license_id.assert_called_once_with(valid_attorney_dto.license_id)

    @pytest.mark.asyncio
    async def test_validate_on_create_phone_taken(self, attorney_validator, attorney_repo_mock, valid_attorney_dto):
        ...
        # Проверяем, что выбрасывается исключение для email
        with pytest.raises(ValidationException, match=re.escape(f'Номер телефона {valid_attorney_dto.phone} уже занят')):
            await attorney_validator.validate_on_create(valid_attorney_dto)
