...
class AttorneyPolicy:
    ...
    async def on_create(self, cmd: RegisterAttorneyCommand) -> None:
        ...
        if existing:
            ...
            raise ValidationException(
                f'Email {cmd.email} уже зарегистрирован'
                )
        ...
        if existing:
            ...
            raise ValidationException(
                f'Номер телефона {cmd.phone} уже занят'
                )
    ...
    async def on_update(self, attorney_id: int, cmd: UpdateAttorneyCommand) -> None:
        ...
        if cmd.license_id and cmd.license_id != attorney.license_id:
            existing = await self.repo.get_by_license_id(cmd.license_id)
            if existing:
                raise ValidationException(f'Номер Удостоверения адвоката {cmd.license_id} уже занят')
