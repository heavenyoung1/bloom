    ...
    async def execute(
        ...
        async with self.uow_factory.create() as uow:
            try:
                ...
                update_cmd = UpdateСlientPaymentCommand(
                    name=cmd.name,
...
class ChangeStatusPaymentUseCase:
    '''Сценарий: юрист обновляет платеж.'''
    ...
    async def execute(
        ...
        async with self.uow_factory.create() as uow:
            try:
                ...
                update_status_cmd = ChangeClientPaymentStatusCommand(
                    payment_id=cmd.payment_id,
                    status=cmd.status,
                )
                payment.change_status(update_status_cmd)
                ...
                logger.error(f'Неизвестная ошибка при обновлении статуса платежа: {e}')
                raise Exception('Ошибка при обновлении статуса платежа')
